...
fig.write_html(result_folder/"pwelch_details.html", config=plotly_config)
...
for st in ["bua", "neuron"]:
  ...
  fig.show(config=plotly_config)
  fig.write_html(result_folder/f"coh_power_{st}.html", config=plotly_config)

# %%
px.histogram(np.abs(all_coh).sel(f=slice(8, 35)).idxmax("f").sel(
    channel_pair=(all_coh["condition"]=="Park") & (all_coh["has_swa"]==False) & (all_coh["is_APO"]==False)
    ).to_dataframe().reset_index(), x="f", facet_col="sig_group_1", facet_row="sig_group_2", histnorm="probability")
...
for st in ["bua", "neuron"]:
  ...
  f1.write_html(result_folder/f"coh_phase_distribution_{st}_details.html", config=plotly_config)
  display(f1)
  f2 = px.line(res.sel(sig_group_1="eeg").to_dataframe(name="power").reset_index().dropna(), 
  ...
  f2.write_html(result_folder/f"coh_phase_distribution_relEEG_{st}.html", config=plotly_config)
  display(f2)
...
for st in ["bua", "neuron"]:
  for method, coh_phase_groups in coh_phase_group_dict.items():
    ...
    fig.show(config=plotly_config)
    fig.write_html(result_folder/f"coh_phase_polarhist_{method}_{st}_details.html", config=plotly_config)
